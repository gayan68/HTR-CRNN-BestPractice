...
class CNN(nn.Module):
    def __init__(self, cnn_cfg, flattening='maxpool'):
        ...
        self.features = nn.ModuleList([nn.Conv2d(1, 32, 7, [2, 2], 3), nn.ReLU()]) #[4, 2]
        in_channels = 32
        cntm = 0
        cnt = 1
...
class CTCtopB(nn.Module):
    def __init__(self, input_size, rnn_cfg, nclasses, rnn_type='gru'):
        ...
        self.fnl = nn.Sequential(nn.Dropout(.2), nn.Linear(2 * hidden, nclasses)) #0.5

        self.cnn = nn.Sequential(nn.Conv2d(input_size, nclasses, kernel_size=(1, 3), stride=1, padding=(0, 1))) #31 #nn.Dropout(.2), 
