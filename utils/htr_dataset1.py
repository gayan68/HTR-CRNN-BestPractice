...
class HTRDatasetBoth(Dataset): 
    def __init__(self, 
        ...
        self.prob_strike = config.probability.strike
            ...
            for line in f:
                ...
                if (config.preprocess.ignore_1_chr == False) or (len(transcr)>1):
                    if (len(config.preprocess.ignore_chars) == 0) or (transcr not in config.preprocess.ignore_chars):
                        ...
                        if rand_spin_for_strike < self.prob_strike:
                            selected_striked = random.choices(striked_inputs, weights=striked_probabilities, k=1)[0]
                            img_path = os.path.join(self.basefolder_strike, subset, "images", selected_striked , img_id + '.png')
                            data += [(img_path, transcr)]
