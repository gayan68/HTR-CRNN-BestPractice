...
class HTRDatasetBoth(Dataset): 
    def __init__(self, 
        ...
        with open(os.path.join(self.basefolder_clean, subset, 'gt.txt'), 'r') as f:
            for line in f:
                strike_bypass, strike_type_bypass = True, True
                ...
                if len(strike_type_arr)>0 :
                    ...
                    strike_bypass = False           #This happens when there is strike found in the dataset (due to lack of data)
                    if (rand_spin_for_strike_type <= striked_types[strike_type]):
                        strike_type_bypass = False  

                if (rand_spin_for_clean <= self.prob_clean) or (strike_bypass) or ((strike_bypass == False) and strike_type_bypass):
